...
"""
import numpy as np# for scientific computing with Python. e.g, array object, linear algebra
import os# provides a way of using operating system dependent functionality like fileopen or path of file
import random#Random variable generators.
import re#Support for regular expressions (RE)that contain both special and ordinary characters
import shutil#Utility functions for copying and archiving files and directory trees.
import time# handle time-related tasks
import tensorflow as tf#library helping in the development of neural networks architecture

import matplotlib.pyplot as plt# makes some change to a plotted figure

from tensorflow.contrib.tensorboard.plugins import projector# a tool that lets you interactively visualize embeddings. This tool can read embeddings from your model and render them in two or three dimensions
...
class LstmRNN(object):
    ...
    def build_graph(self):
        ...
        #Allowing it to be random enables data augmentation which prevents overfitting or "Cramming"
    ...
    def train(self, dataset_list, config):
        ...
        #Logs allow storage of activities and changes in running of codes
        ...
        #First kept at zero then updated with time
        global_step = 0
        ...
        for epoch in range(config.max_epoch):
            #Initialized to zero first so that we may update it later
            epoch_step = 0
    ...
    #Saves the model to disk, and also allows loading it from disk
    def load(self):
